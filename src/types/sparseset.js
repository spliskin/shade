class SparseSet extends Array {
    ...
    constructor(size=null) {
        ...
        this._capacity = 0; // Current allocated capacity

        if (Array.isArray(size)) {
            this.accomodate(size.length);
	...
	add(obj) {
		...
		if (!this.has(val)) {
			if (val >= this._capacity)
                this.reserve(this.resizepolicy(this._capacity));
...
*/
    ...
    subset(other) {
        // if size of this set is greater than otherSet then it can'nt be  a subset
    ...
    intersection(s) {
        ...
        //const [ a, b ] = this.size() <= s.size() ? [ this, s ] : [ s, this ];
        //var result = new SparseSet;
        //for(var i=0, m=b.size(); i < m; i++) {
        //    if (a.has(b[i]))
        //        result.add(b[i]);
        //}
    ...
    union(s) {
        ...
        //set.reserve(Math.max(s.size(), this.size()));
